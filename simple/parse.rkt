...
(define (parse-axiom str name line col pos)
  ...
  (define-values (axiom last-axiom-char) (parse-sequence-of-symbols the-port name))
  axiom)
...
(define (parse-rule str name line col pos failed)
  ...
  (port-count-lines! the-port)
  (set-port-next-location! the-port line col pos)
  (define-values (left last-left-char) (parse-sequence-of-symbols the-port name end?))
  (define-values (right last-right-char) (parse-sequence-of-symbols the-port name))
  (when (empty? left)
    (failed "expected the name of a non-terminal"))
  (when (> (length left) 1)
    (failed
     (format "expected exactly 1 non-terminal name before ~a" (if arr1? "->" "→"))))
  (rule (first left) right))

(define (parse-sequence-of-symbols sp name [end? (λ (c p) (eof-object? c))])
  (let loop ([stxs '()])
